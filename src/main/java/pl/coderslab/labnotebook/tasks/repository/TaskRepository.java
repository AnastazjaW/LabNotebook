...
public interface TaskRepository extends JpaRepository<Task, Long> {
    @Query(value = "select * from tasks t  join users_projects as up on t.project_id = up.project_id where up.user_id = :userId and t.is_finished = 0 order by t.last_modification_date desc", nativeQuery = true)
